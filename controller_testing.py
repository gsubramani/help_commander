...
hEC.send_parameter(axis, PARAMETER_POSITION_GAIN,0, 300)
hEC.send_parameter(axis, PARAMETER_INTEGRATOR_GAIN,0, 5)
hEC.send_parameter(axis, PARAMETER_INTEGRATOR_CLAMP,0, 3)
hEC.send_parameter(axis, PARAMETER_DAMPING,0, 50)

hEC.send_parameter(axis, PARAMETER_INPUT_SETPOINT,0, 0.1)
# hEC.send_parameter(axis, PARAMETER_INPUT_AMPLITUDE,0, 0.2)
# hEC.send_parameter(axis, PARAMETER_INPUT_FREQUENCY,0, 0.03)
...
some_data = hEC.get_stream_data(18000,clear_old = False)



hEC.send_parameter(axis, PARAMETER_CLEAR,0, 0);
hEC.send_parameter(axis, PARAMETER_TORQUE_CLAMP,0, 10.0)
hEC.send_parameter(axis, PARAMETER_VELOCITY_LIMIT,0, 0.5)

hEC.send_parameter(axis, PARAMETER_FRICTION_DITHER_RATE,0, 0)
hEC.send_parameter(axis, PARAMETER_FRICTION_FF_STATIC,0, 0)

hEC.send_parameter(axis, PARAMETER_POSITION_GAIN,0, 20)
...
hEC.send_parameter(axis, PARAMETER_INPUT_SETPOINT,0, 0.3)
hEC.send_parameter(axis, PARAMETER_COMMIT,0, 0);
